(function(){
 ...
 panel.querySelector('.punch-footer .hint').textContent='Type to see suggestions · Enter or Tab to add · Select for checkboxes';
 ...
 scroll.insertAdjacentHTML('beforebegin','<div class="punch-copy-tools"><button class="action" id="punchSelectCells" aria-pressed="false">Select cells</button><label><input id="punchCopyHeaders" type="checkbox" checked> Include headers</label><button class="action" id="punchCopyCells" disabled>Copy selected</button><button class="action" id="punchExportSelected" disabled>Export selected punches (.xlsx)</button><button class="action" id="punchExportFiltered">Export filtered (.xlsx)</button><button class="action punch-quick-filter" id="punchQuickFilter" title="Open Electrical / I&amp;C punches in category A or B; keeps your subsystem filter">Electrical A/B · Open</button><button class="action" id="punchImportPhotos" hidden>Import photos</button><span id="punchCopyHint" class="hint" role="status">Click a cell, then Shift-click the opposite corner. Esc cancels selection.</span></div>');
 $('punchQuickFilter').onclick=()=>{for(const k of ['statusText','discipline','category']){delete filters[k];delete selectedFilters[k];delete exactFilters[k]}selectedFilters.statusText=['Open'];selectedFilters.category=['A','B'];exactFilters.discipline=[...new Set((PunchItems.getSnapshot()?.rows||[]).filter(r=>PunchItems.disciplineTone(r)==='disc-electric').map(r=>PunchItems.discipline(r)))];makeColumns();scroll.scrollTop=0;render(query,true)};
 ProjectAccess.ready.then(u=>{$('punchImportPhotos').hidden=!['writer','admin'].includes(u.role)});$('punchImportPhotos').onclick=()=>window.PunchExtras?.bulkUpload();
 ...
 $('punchMobileCards').before(panel.querySelector('.punch-copy-tools'));
 ...
 function fitColumns(){
  const widths=columns.map(([key,label])=>{
   if(key==='latestComment')return 220;if(key==='cableCompletion')return 170;if(key==='photoCount')return 76;if(key==='statusText')return 82;if(key==='description')return 350;
 ...
 const suggestions=document.createElement('div');suggestions.id='punchSuggestions';suggestions.className='punch-suggestions';suggestions.hidden=true;suggestions.setAttribute('role','listbox');document.body.append(suggestions);
 let suggestionInput=null,suggestionValues=[],suggestionIndex=0;
 function hideSuggestions(){suggestions.hidden=true;if(suggestionInput){suggestionInput.setAttribute('aria-expanded','false');suggestionInput.removeAttribute('aria-activedescendant')}suggestionInput=null}
 function paintSuggestions(){suggestions.innerHTML=suggestionValues.map((v,i)=>'<div role="option" id="punchSuggestion'+i+'" aria-selected="'+(i===suggestionIndex)+'" data-suggestion="'+i+'" title="'+esc(v)+'">'+esc(v)+'</div>').join('');suggestionInput?.setAttribute('aria-activedescendant','punchSuggestion'+suggestionIndex);suggestions.children[suggestionIndex]?.scrollIntoView({block:'nearest'})}
 function showSuggestions(input){const key=input.dataset.punchFilter,term=input.value.trim();if(!key||!term){hideSuggestions();return}const wanted=new Set(tokens),found=new Set();for(const row of PunchItems.getSnapshot()?.rows||[]){const number=norm(row.itemNumber);if(wanted.size&&!wanted.has(number)||query&&!number.includes(norm(query))||!matchesFilters(row,key))continue;const v=String(value(row,key));if(!v||(selectedFilters[key]||[]).includes(v))continue;if(norm(v).includes(norm(term))||(['closedDate','issuedDate'].includes(key)&&PunchItems.dateMatches(v,term)))found.add(v)}suggestionValues=[...found].sort((a,b)=>collator.compare(a,b)).slice(0,100);if(!suggestionValues.length){hideSuggestions();return}suggestionInput=input;suggestionIndex=0;input.setAttribute('role','combobox');input.setAttribute('aria-autocomplete','list');input.setAttribute('aria-controls','punchSuggestions');input.setAttribute('aria-expanded','true');const r=input.getBoundingClientRect(),width=Math.min(Math.max(r.width,250),innerWidth-16);suggestions.style.width=width+'px';suggestions.style.left=Math.max(8,Math.min(r.left,innerWidth-width-8))+'px';const below=innerHeight-r.bottom-8;suggestions.style.maxHeight=Math.max(72,Math.min(250,below>110?below:r.top-8))+'px';suggestions.style.top=below>110?r.bottom+3+'px':'auto';suggestions.style.bottom=below>110?'auto':innerHeight-r.top+3+'px';suggestions.hidden=false;paintSuggestions()}
 function acceptSuggestion(){if(!suggestionInput||suggestions.hidden)return;const key=suggestionInput.dataset.punchFilter,term=suggestionValues[suggestionIndex];suggestedExact.add(JSON.stringify([key,term]));const selected=selectedFilters[key]||(selectedFilters[key]=[]);if(!selected.includes(term))selected.push(term);filters[key]='';hideSuggestions();makeColumns();scroll.scrollTop=0;render(query,true);[...$('punchHead').querySelectorAll('input')].find(x=>x.dataset.punchFilter===key)?.focus({preventScroll:true})}
 panel.addEventListener('keydown',event=>{const input=event.target;if(!input.dataset.punchFilter||event.isComposing)return;if(event.key==='ArrowDown'||event.key==='ArrowUp'){event.preventDefault();event.stopImmediatePropagation();if(suggestions.hidden||suggestionInput!==input)showSuggestions(input);else{suggestionIndex=(suggestionIndex+(event.key==='ArrowDown'?1:-1)+suggestionValues.length)%suggestionValues.length;paintSuggestions()}}else if(!suggestions.hidden&&suggestionInput===input&&['Enter','Tab'].includes(event.key)){event.preventDefault();event.stopImmediatePropagation();acceptSuggestion()}else if(event.key==='Escape'&&!suggestions.hidden){event.preventDefault();event.stopImmediatePropagation();hideSuggestions()}},true);
 panel.addEventListener('focusin',event=>{if(event.target.dataset.punchFilter)showSuggestions(event.target)});
 panel.addEventListener('focusout',event=>{if(event.target===suggestionInput)hideSuggestions()});
 suggestions.addEventListener('mousedown',event=>event.preventDefault());suggestions.addEventListener('click',event=>{const option=event.target.closest('[data-suggestion]');if(option){suggestionIndex=+option.dataset.suggestion;acceptSuggestion()}});
