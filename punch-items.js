(function(root){
 ...
 const api={findItems,searchItems,rowStatus,dateKey,formatDate,dateMatches,discipline,extractTags,relatedItems,mergeSnapshot};
 ...
 function openDetails(title,items,selected=null){
  ...
  target.innerHTML='<p>'+items.length+' punch items'+(selected?' · Related through matching tags in Location / Room or Description':'')+'</p><label>Discipline <select id="punchDetailDiscipline"><option value="">All disciplines</option>'+disciplines.map(d=>'<option>'+escape(d)+'</option>').join('')+'</select></label><div class="wrap punch-detail-scroll"><table><thead><tr><th>Punch Item No.</th><th>Discipline</th><th>Description</th><th>Location / Room</th><th>Status</th><th>Closed Date</th></tr></thead><tbody id="punchDetailRows"></tbody></table></div><button id="punchDetailMore" class="action">Show more</button><div id="punchDetailCount" class="hint"></div>';
  function paint(){const choice=document.getElementById('punchDetailDiscipline').value,found=items.filter(r=>!choice||discipline(r)===choice);
   document.getElementById('punchDetailRows').innerHTML=found.slice(0,limit).map(x=>{const state=rowStatus(x);return '<tr'+(x===selected?' class="punch-selected"':'')+'><td>'+escape(x.itemNumber)+(x===selected?'<div class="hint">Selected item</div>':'')+'</td><td>'+escape(discipline(x)||'—')+'</td><td>'+escape(x.description||'—')+'<p><strong>Subsystem:</strong> '+escape(x.subsystem||'—')+' · <strong>Category:</strong> '+escape(x.category||'—')+'</p></td><td>'+escape(x.locationRoom||'—')+'</td><td>'+escape(state.text)+'</td><td>'+escape(formatDate(x.closedDate)||'—')+'</td></tr>'}).join('');
