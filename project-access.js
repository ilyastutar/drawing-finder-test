...
async function parse(response){let d;try{d=await response.json()}catch{throw Error('Server response could not be read.')}if(!response.ok||d.error){const code=d.error?.message||d.error||'Request failed';throw Error(code)}return d}
...
$('accessRegister').onclick=()=>{registering=!registering;$('accessNameLabel').hidden=!registering;$('accessSubmit').textContent=registering?'Create account':'Sign in';$('accessRegister').textContent=registering?'Back to sign in':'Create account';$('accessPassword').autocomplete=registering?'new-password':'current-password';$('accessMessage').textContent=registering?'New accounts require email verification and administrator approval.':''};
